...
#### P3B-4c 🟡（部分完成：Rayleigh 大气散射；四叉树 LOD 后续）
- **Rayleigh + Mie 单次散射（已完成）**：`atmosphere.wgsl` 从 Fresnel-rim 升级为
  物理启发的散射模型
  - 波长依赖 Rayleigh 系数 β_R = (5.8, 13.6, 33.1)×10⁻⁶ m⁻¹（Earth 参考）
  - Mie 散射系数 β_M = 21×10⁻⁶ m⁻¹（各向异性 g=0.76）
  - Rayleigh 相位函数 P_R = 3(1+cos²θ)/16π
  - Henyey-Greenstein Mie 相位（前向散射峰值 → 日落 halo）
  - 视线路径长度 ≈ H_R/cos(view_zenith)（Chapman 近似）
  - 太阳路径透射 T_sun = exp(-β_R × H_R / cos(sun_zenith)) → 日落时蓝光被削减，
    留下橙红（终点线视觉正确）
  - 昼夜过渡 smoothstep(-0.1, 0.15, mu_s) 平滑淡出
  - 每天体 atmo_color.rgb 作为 β_R bias（Earth 蓝 / Mars 红 / Venus 黄 / Titan 橙）
  - Alpha = 亮度 clamp[0,1]：中心透明，limb 不透明
- 四叉树瓦片 LOD（TileManager2 / ZTreeMgr / elevmgr 移植 + 高程数据）—— 后续


### P3C — 航天器渲染 🟡（P3C-1/2/3/4a 已完成，P3C-4b 后续）
...
#### P3C-4a PBR-ish shader ✅
- `crates/orbitx-app/src/shader/vessel.wgsl`：简化 Cook-Torrance BRDF：
  * Lambert 漫反射 × (1 - metallic)（金属不吸收漫反射能量）
  * GGX / Trowbridge-Reitz 法线分布函数
  * Schlick Fresnel（F₀ 由 metallic 混合 dielectric 0.04 与 base_color）
  * Smith-Schlick 几何衰减近似（k = (r+1)² / 8）
  * Fresnel rim 边缘高光（金属加权）
  * 环境光 base × 0.10 避免阴影侧全黑
- 复用 `Uniforms` 布局（同 BGL / 顶点格式），两个字段 repurpose：
  * `light_dir.w` = metallic
  * `log_depth.w` = roughness
- 独立 `vessel_pipeline`（复用 `s_layout` + `Vertex` 格式）
- `VesselRenderState { metallic, roughness }` 默认 (0.85, 0.35) —— 刷铝质感
- `BodyDraw::VesselMesh` 携带 metallic/roughness
- 相机在 render 空间原点，`v = normalize(-world_pos)` 计算视方向
...
- **不改** `orbitx-cli` 接线（cli 暂继续用旧 `control` 模块，**P4.3 已退役进程内步进**）。
...
- **CommsService** = **真**本机 Zenoh + SHM；线载荷 **protobuf**（crate [`orbitx-protocol`](../crates/orbitx-protocol)）；经既有 flume 对接 Runtime，不改编排。P4.2 Comms stub **已删除**（无双路径）。
- **`orbitx-cli`** = 同机 Zenoh 客户端；自 `target/<profile>` **spawn** 同 profile 的 `orbitx-runtime`，**退出时 Shutdown + kill** 子进程，无残留。
- 废弃 cli 进程内 `Assembly` 步进 / `control.rs` TVC·节流；Controller **仅**在 Runtime 线程内调用。
- **`TargetMode::GravityTurn`**：标准 kick→α≈0（推力∥速度）；非高度表 / 非 `pitch_rate` 斜坡。API：`{ throttle, kick_angle, kick_rate }`。
- **台架钉死/松绑**：P4.3 **先在 Runtime 会话策略**（过渡）；终态 → 发射台操作插件注入步进运行时（另排）。
- **坠毁**：P4.3 **先在 Runtime**（现 `alt≤0` 启发式）；终态 → **P5** 真实高程/接触后权威判定。
- **自动分离**：**不进 Controller**；cli 读 Slice 遥测 → 发 `InputCmd::Separate`（仅 Primary 焦点）；终态 → WorkFlow/客户端编排。
...
- 复用 P4.3 **本机** Zenoh + SHM 与 **`.proto` / `orbitx-protocol`**；`sim-rocket/rust` bridge 对齐 orbitx（替换 NullBridge）。
...
- **收回 P4.3 Runtime 过渡硬坠毁**（`alt≤0` 启发式），以真实高程/接触为权威。
