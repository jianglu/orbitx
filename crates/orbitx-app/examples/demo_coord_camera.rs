//! Demo 4: verifies the complete coordinate transform chain from orbitx-app.
//!
//! Simulates the full pipeline: hardcoded sim positions -> CameraSystem update
//! -> CoordinateBridge conversion -> sphere/billboard decision -> rendering.
//! Demos 1-3 (triangle, billboard, sphere) all passed; this combines their
//! rendering paths with the real coordinate transform chain.
//!
//! Run with:  cargo run -p orbitx-app --example demo_coord_camera
...
fn estimate_screen_pixels(
    ...
    // Project center
    let center_clip =
        *view_proj * glam::Vec4::new(render_pos.x, render_pos.y, render_pos.z, 1.0);
    if center_clip.w <= 0.0 {
        return 0.0; // Behind camera
    }

    // Project a point offset by render_radius perpendicular to the view direction
    let offset_dir = if render_pos.x.abs() > 0.001 {
        glam::Vec3::new(0.0, 1.0, 0.0)
    } else {
        glam::Vec3::new(1.0, 0.0, 0.0)
    };
    let edge_pos = render_pos + offset_dir * render_radius;
    let edge_clip = *view_proj * glam::Vec4::new(edge_pos.x, edge_pos.y, edge_pos.z, 1.0);
    if edge_clip.w <= 0.0 {
        return 0.0;
    }

    // NDC difference -> pixel difference
    let center_ndc_x = center_clip.x / center_clip.w;
    let edge_ndc_x = edge_clip.x / edge_clip.w;
    let ndc_diff = (edge_ndc_x - center_ndc_x).abs();

    ndc_diff * vp_width * 0.5
}
...
impl App {
    ...
    fn render(&mut self) {
        ...
        let cam_pos = self.coord_state.cam_pos_sim;
        let render_scale = self.coord_state.render_scale;
        ...
        let full_output = self.egui_ctx.run_ui(egui_input, |ui| {
            ...
            egui::CentralPanel::default()
                ...
                .show(ui, |ui| {
                    ui.label("CoordinateBridge + Camera Demo");
                    ui.label(format!(
                        "Camera pos (sim): ({:.3e}, {:.3e}, {:.3e})",
                        cam_pos.x, cam_pos.y, cam_pos.z
                    ));
                    ui.label(format!("Render scale: {:.6e}", render_scale));
                    ui.separator();
                    for (i, body) in BODIES.iter().enumerate() {
