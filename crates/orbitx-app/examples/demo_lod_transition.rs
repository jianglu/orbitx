//! Demo 6: verifies the LOD sphere-vs-billboard transition as the camera zooms.
//!
//! A single Earth-like body is rendered while the camera distance oscillates in
//! log space between near and far. As the projected screen size of the sphere
//! crosses the `min_render_px` threshold, the render mode flips between
//! Billboard (far away) and Sphere (close up), demonstrating the LOD transition.
//!
//! This is a minimal adaptation of demo_coord_camera.rs (Demo 4): single body,
//! animated camera distance, continuous redraw, and an updated UI overlay.
//!
//! Run with:  cargo run -p orbitx-app --example demo_lod_transition
...
/// Earth radius in meters (for UI display).
...
impl CoordState {
    ...
    ///
    /// `dist` is the animated camera distance for this frame; it is applied to
    /// the external camera mode before the camera update so the LOD decision
    /// reflects the current zoom level.
    fn update(&mut self, vp_width: f32, vp_height: f32, dist: f64) {
        // Apply the animated camera distance for this frame.
        self.camera.set_ext_mode(ExternalCamMode::TargetRelative {
            dist,
            phi: 0.0,
            theta: 0.3,
        });
    ...
    fn sphere_uniforms(&self, body_idx: usize) -> Uniforms {
        ...
        // Light direction from sun render position (body 0 in this demo)
...
struct App {
    ...
    running: bool,
    /// Start time for the animated camera distance.
...
impl App {
    ...
    fn render(&mut self) {
        ...
        // Animated camera distance: oscillate in log space for a smooth zoom.
        let t = self.start.elapsed().as_secs_f64();
        let phase = (t * 0.3).sin() * 0.5 + 0.5; // 0..1 oscillation
        let log_far = (1.0e10f64).ln();
        let log_near = (2.0e7f64).ln();
        let dist = (log_near + (log_far - log_near) * phase).exp();

        // Update coordinate state each frame with the animated distance
        self.coord_state.update(vp_w, vp_h, dist);
        ...
        // Snapshot body modes for UI and callback
        ...
        let full_output = self.egui_ctx.run_ui(egui_input, |ui| {
            ...
            egui::CentralPanel::default()
                ...
                .show(ui, |ui| {
                    ui.label("Demo 6: LOD Transition (billboard <-> sphere)");
                    ui.label(format!(
                        "Camera distance: {:.3e} m  ({:.2} Earth radii)",
                        dist,
                        dist / EARTH_RADIUS_M
                    ));
                    let mode_str = if body_modes[0] == RenderMode::Billboard {
                        "BILLBOARD"
                    } else {
                        "SPHERE (3D)"
                    };
                    ui.label(format!("Render mode: {}", mode_str));
                    ui.separator();
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // Reset the animation clock when the window is (re)created.
        self.start = std::time::Instant::now();

        let window_attrs = WindowAttributes::default()
            .with_title("demo_lod_transition")
